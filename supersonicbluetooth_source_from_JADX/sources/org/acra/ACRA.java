...
public class ACRA {
    ...
    static class C04351 implements OnSharedPreferenceChangeListener {
        C04351() {
        }
    ...
    public static void init(Application app) {
        ...
        if (mReportsCrashes == null) {
            log.mo1318e(LOG_TAG, "ACRA#init called but no ReportsCrashes annotation on Application " + mApplication.getPackageName());
            return;
        }
        SharedPreferences prefs = getACRASharedPreferences();
        try {
            checkCrashResources();
            log.mo1316d(LOG_TAG, "ACRA is enabled for " + mApplication.getPackageName() + ", intializing...");
            ...
            log.mo1326w(LOG_TAG, "Error : ", e);
        }
        mPrefListener = new C04351();
    ...
    public static ACRAConfiguration getConfig() {
        if (configProxy == null) {
            if (mApplication == null) {
                log.mo1325w(LOG_TAG, "Calling ACRA.getConfig() before ACRA.init() gives you an empty configuration instance. You might prefer calling ACRA.getNewDefaultConfig(Application) to get an instance with default values taken from a @ReportsCrashes annotation.");
            }
