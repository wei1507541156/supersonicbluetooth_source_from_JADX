...
public abstract class Sprite extends VisibleComponent implements AlarmHandler, OnDestroyListener, Deleteable {
    ...
    public void PointTowards(Sprite target) {
        Heading(-Math.toDegrees(Math.atan2((target.m27Y() - m27Y()) + ((double) ((target.Height() - Height()) / 2)), (target.m25X() - m25X()) + ((double) ((target.Width() - Width()) / 2)))));
    }
    ...
    public void PointInDirection(double x, double y) {
        Heading(-Math.toDegrees(Math.atan2((y - m27Y()) - ((double) (Height() / 2)), (x - m25X()) - ((double) (Width() / 2)))));
    }
    ...
    public BoundingBox getBoundingBox(int border) {
        return new BoundingBox(m25X() - ((double) border), m27Y() - ((double) border), ((m25X() + ((double) Width())) - DEFAULT_Z) + ((double) border), ((m27Y() + ((double) Height())) - DEFAULT_Z) + ((double) border));
    }
