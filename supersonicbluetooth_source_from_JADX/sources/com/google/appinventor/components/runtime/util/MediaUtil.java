...
public class MediaUtil {
    ...
    public static boolean isExternalFileUrl(String mediaPath) {
        return mediaPath.startsWith("file:///sdcard/") || mediaPath.startsWith("file://" + Environment.getExternalStorageDirectory().getAbsolutePath());
    }
        ...
        AsynchUtil.runAsynchronously(new Runnable() {
            public void run() {
                ...
                try {
                    ...
                    continuation.onFailure("PERMISSION_DENIED:" + e5.getPermissionNeeded());
                    if (is != null) {
                        try {
                            is.close();
                        } catch (IOException e32222) {
                            Log.w(MediaUtil.LOG_TAG, "Unexpected error on close", e32222);
                        }
                    }
                    try {
                        bos.close();
                    ...
                    Log.d(MediaUtil.LOG_TAG, "IOException reading file.", e3222222);
                    continuation.onFailure(e3222222.getMessage());
                    if (is != null) {
                        try {
                            is.close();
                    ...
                    if (is != null) {
                        try {
                            ...
                            Log.w(MediaUtil.LOG_TAG, "Unexpected error on close", e322222222);
                        }
                    }
                    try {
                        bos.close();
