...
public class TelnetRepl extends Procedure0 {
    ...
    public static Thread serve(Language language, Socket client) throws IOException {
        ...
        Thread thread = new BiggerFuture(new TelnetRepl(language, client), new TtyInPort(sin, FilePath.valueOf("/dev/stdin"), out), out, out, "Telnet Repl Thread", PlaybackStateCompat.ACTION_SET_REPEAT_MODE);
        thread.start();
        return thread;
    }
}
