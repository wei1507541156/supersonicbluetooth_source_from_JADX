...
public class PrimProcedure extends MethodProc implements Inlineable {
    ...
    private void init(Method method) {
        ...
        if ((method.getModifiers() & 8) != 0) {
            this.op_code = YaVersion.YOUNG_ANDROID_VERSION;
        } else {
    ...
    public PrimProcedure(int op_code, ClassType classtype, String name, Type retType, Type[] argTypes) {
        ...
        this.method = classtype.addMethod(name, op_code == YaVersion.YOUNG_ANDROID_VERSION ? 8 : 0, argTypes, retType);
        this.retType = retType;
        this.argTypes = argTypes;
        if (op_code != YaVersion.YOUNG_ANDROID_VERSION) {
            c = 'V';
        }
        this.mode = c;
    }
